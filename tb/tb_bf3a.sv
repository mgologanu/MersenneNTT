...
module tb_bf3a;
  ...
  function automatic void compute_expected(
    ...
    //conj(sqrt(i)) = conj(1+i)*2^((W-1)/2) = (1-i)*2^((W-1)/2)
    z1.re = FACTOR;
    z1.im = p-FACTOR;


     
     a_exp = cmpl_add_mod(a, b, p);
     z2    = cmpl_sub_mod(a, b, p); 

     b_exp = cmpl_mult_mod(z2, z1, p);
  ...
  initial begin
    ...
    $display("bf3a testbench PASSED for W=%0d, %0d random vectors.", W, 2000);
    $finish;
  end
